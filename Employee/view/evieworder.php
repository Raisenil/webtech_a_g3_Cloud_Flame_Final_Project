				...
				<table border="1">
					...
					<?php  for($i=0; $i<count($users); $i++){ ?>
						<tr>
							...
							<td><center>
								<a class="btn btn-small btn-small-primary" href="eeditorder.php?
								id=<?=$users[$i]['id']?>
								&food=<?=$users[$i]['food']?>
								&price=<?=$users[$i]['price']?>
								&qty=<?=$users[$i]['qty']?>
							...
							<td>
								<center><a class="btn btn-red btn-red-primary" href="../model/edeleteorder.php?id=<?=$users[$i]['id']?>">Delete</a>
					</center>
							</td>
						</tr>
					<?php } ?>
